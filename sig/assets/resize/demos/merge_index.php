<?php
  ...
  **/
	...
	$top = Request::getRegex('top', Registry::get('image dim regex'), '25');
	$palette = Request::getInt('palette');
...
<table>
	<tr>
		<td>
			Left: <input type="text" size="5" name="left" value="<?php echo $left; ?>" />
			Top: <input type="text" size="5" name="top" value="<?php echo $top; ?>" />
			Percent: <input type="text" size="5" name="pct" id="pct" value="<?php echo $pct; ?>" />
...
?>
			...
			<input type="checkbox" name="palette" id="palette" <?php if ($palette) echo 'checked="checked"'; ?> value="1" />
			<label for="palette">asPalette</label>
		</td>
		<td>
...
<?php
	...
	foreach (Registry::get('images') as $image)
	{
		...
		$img_url = "merge.php?img={$image}&overlay={$overlay}&left={$left}&top={$top}&pct={$pct}&palette={$palette}";
?>
		<td class="right">
