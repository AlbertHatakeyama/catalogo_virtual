<?php
  ...
  **/
	...
	Registry::set('resize scale', array('any', 'down', 'up'));
	Registry::set('image dim regex', '/^((c([+-]?[0-9]+(\.[0-9]+)?)?\%?)|([+-]?[0-9]+(\.[0-9]+)?\%?))$/');
	Registry::set('int', '/^[0-9]+$/');
	Registry::set('rgb regex', '/([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})/i');
...
<style>
	...
	<body>
		<p class="navbar">
			...
			<a href="autocrop_index.php">autocrop</a>
