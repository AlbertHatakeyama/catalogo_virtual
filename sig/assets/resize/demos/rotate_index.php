...
<table>
	<tr>
		<td>
			...
			<input type="button" value="+" onclick="document.getElementById('angle').value = document.getElementById('angle').value*1 + 5; document.params.submit();" />
			<input type="button" value="-" onclick="document.getElementById('angle').value = document.getElementById('angle').value*1 - 5; document.params.submit();" />
...
<?php
	...
	foreach (Registry::get('images') as $image)
	{
		...
		$img_url = "rotate.php?img={$image}&angle={$angle}";
?>
		<td class="right">
