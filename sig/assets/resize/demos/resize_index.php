...
<?php
	...
	foreach (Registry::get('resize scale') as $scale_option)
	{
		if ($scale_option == $scale)
			$sel = 'selected="selected"';
		else
			$sel = '';
		echo "<option {$sel} value=\"{$scale_option}\">{$scale_option}</option>\n";
	}
?>
	</select>
		</td>
...
<?php
	...
	foreach (Registry::get('images') as $image)
	{
		...
		$img_url = "resize.php?img={$image}&width={$width}&height={$height}&fit={$fit}&scale={$scale}";
?>
		<td class="right">
